...
namespace MoveFilesTests
{
    public class ArchiveTests : ReaderTests
    {
        protected void ArchiveStreamReadExtractAll(string testArchive, CompressionType compression)
        {
            ...
            List<string> testEnumerable = new List<string> { testArchive };
            ArchiveStreamReadExtractAll(testEnumerable, compression);
        }
        ...
        {
            testArchive = Path.Combine(TEST_ARCHIVES_PATH, testArchive);
            // .net didn't like the original implementation of just doing testArchive.AsEnumerable();
            List<string> testEnumerable = new List<string> { testArchive };
            ArchiveStreamRead(readerOptions, testEnumerable);
        }
